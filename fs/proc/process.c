...
errno_t proc_process_open ( proc_snap_t *snap, ino_t inode ) {
	...
	proc_snap_appenddir( snap, ".."    , PROC_INODE(   0, 0x000 ) );
	proc_snap_appenddir( snap, "tasks" , PROC_INODE( pid, 0x004 ) );
	proc_snap_appenddir( snap, "name"  , PROC_INODE( pid, 0x002 ) );
	proc_snap_appenddir( snap, "state" , PROC_INODE( pid, 0x003 ) );
	return 0;
}
...
errno_t proc_proc_tasks_open ( proc_snap_t *snap, ino_t inode ) {
	...
	for (_t  = p->tasks.next; _t != &p->tasks; _t  = _t->next) {
		...
		proc_snap_appenddir( snap, name, PROC_INODE(t->tid, 0x005) );
	}
	return 0;
}
...
errno_t proc_proc_state_open ( proc_snap_t *snap, ino_t inode ) {
	...
	proc_snap_setline( snap, procstates[p->state] );
