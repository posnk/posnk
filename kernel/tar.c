...
int tar_read_record_mem(uintptr_t tar_data, off_t *pos)
{
	...
	if (strcmp(header->magic, TMAGIC) != 0) {
		printf(CON_WARN, "invalid tar record for file %s magic:\"%s\" read:%i pos:%i",header->name,header->magic,*pos);
		if (strlen(header->name) <= 1) {
			return EIO;
		}

	}
//	debugcon_printf("untar %s %c ",header->name, header->typeflag);
	switch(header->typeflag){
		default:
		case AREGTYPE:
		case REGTYPE:
		case CONTTYPE:
			...
			if (status) {
				printf(CON_WARN, "error creating file while extracting %s, errno: %i",header->name, status);
				return status;
			}
			status = vfs_find_inode(header->name, &file);
			if (status) {
				printf(CON_WARN, "error opening file while extracting %s, errno: %i",header->name, status);
				return status;
			}
			...
			if (file_size != 0) {
				...
				if (status) {
					printf(CON_WARN, "write error while extracting %s",header->name);
					return status;
				}
				if (wd_count !=  (size_t) file_size) {
					printf(CON_WARN, "out of space while extracting %s",header->name);
					return ENOSPC;
				}
			}
		...
		case SYMTYPE://TODO: Implement proper symlinks
			...
			if (status) {
				printf(CON_WARN, "error symlinking %s",header->name);
				return status;
			}
			break;
		case LNKTYPE:
			...
			if (status) {
				printf(CON_WARN, "error linking %s",header->name);
				return status;
			}
			break;
		case CHRTYPE:
		...
		case FIFOTYPE:
			...
			if (status) {
				printf(CON_WARN, "error mknod %s",header->name);
				return status;
			}
			break;
		case DIRTYPE:
			...
			if (status) {
				printf(CON_WARN, "error creating directory while extracting %s, errno: %i",header->name, status);
				return status;
			}
			break;
	...
	if ((header->typeflag != LNKTYPE) && (header->typeflag != SYMTYPE)) {
		status = vfs_find_inode(header->name, &file);
		if (status) {
			printf(CON_WARN, "error opening node while extracting %s, errno: %i",header->name, status);
			return status;
		}
...
int tar_read_record(inode_t *tar_file, off_t *pos)
{
	...
	if (status) {
		...
		printf(CON_WARN, "error read file while extracting , errno: %i",status);
		return status;
	}
	(*pos) += rd_count;
	if (rd_count != sizeof(tar_header_t)) {
		heapmm_free(header, sizeof(tar_header_t));
		printf(CON_WARN, "size mismatch read file while extracting read:%i pos:%i, errno: %i",status,rd_count,*pos);
		return EIO;
	}
	if (strlen(header->name) == 0){
		heapmm_free(header, sizeof(tar_header_t));
	...
	if (strcmp(header->magic, TMAGIC) != 0) {
		printf(CON_WARN, "invalid tar record for file %s magic:\"%s\" read:%i pos:%i",header->name,header->magic,rd_count,*pos);
		if (strlen(header->name) <= 1) {
			heapmm_free(header, sizeof(tar_header_t));
			return EIO;
		}
			...
			if (file->size != 0) {
				...
				for (wr_count = 0; wr_count < file->size; wr_count += e_block_size) {
					...
					if (status) {
						...
						printf(CON_WARN, "read error while extracting %s",header->name);
						return status;
					}
					(*pos) += rd_count;
					if (rd_count != (size_t) e_block_size) {
						printf(CON_WARN, "hit EOF while extracting %s",header->name);
						heapmm_free(header, sizeof(tar_header_t));
						heapmm_free(buf, block_size);
						return EIO;
					}
