...
SYSCALL_DEF3(execve)
{
	...
	for (ct = 0; ct < argvc - 1; ct++) {
		...
		if (( sc < 0 ) || !(argv[ct] = heapmm_alloc(sc))) {
			printf(CON_TRACE, "alloc %x %i",user_str,sc);
			argvc = ct;
			syscall_errno = EFAULT;
			goto fault_e;
		}
		if (!copy_user_to_kern(user_str, argv[ct], sc)) {
			printf(CON_TRACE, "copy");
			argvc = ct + 1;
			memset ( argv[ct], ' ', sc );
			argv[ct][sc - 1] = 0;
			syscall_errno = EFAULT;
